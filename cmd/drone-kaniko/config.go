...
func settingsFlags() []cli.Flag {
	...
	return []cli.Flag{
		...
		&cli.BoolFlag{
			...
			Usage:   `Caches run layers (default true)`,
			EnvVars: []string{"PLUGIN_CACHE_RUN_LAYERS"},
			Value:   true,
		},
		&cli.DurationFlag{
			Name:    "cache-ttl",
		...
		&cli.IntFlag{
			...
			Usage:   `Number of retries for downloading the remote image`,
			EnvVars: []string{"PLUGIN_IMAGE_DOWNLOAD_RETRY"},
		},
		&cli.IntFlag{
		...
		&cli.BoolFlag{
			Name:    "push-ignore-immutable-tag-errors",
			Usage:   `If true, known tag immutability errors are ignored and the push finishes with success.`,
			EnvVars: []string{"PLUGIN_PUSH_IGNORE_IMMUTABLE_TAG_ERRORS"},
		},
		&cli.IntFlag{
			Name:    "push-retry",
		...
		&cli.StringSliceFlag{
			Name:    "registry-map",
			Usage:   `Registry map of mirror to use as pull-through cache instead. Expected format is 'original.registry=new.registry;other-original.registry=other-remap.registry'`,
			EnvVars: []string{"PLUGIN_REGISTRY_MAP"},
		},
		&cli.StringFlag{
...
func settingsFromContext(ctx *cli.Context) kaniko.Settings {
	return kaniko.Settings{
		...
		CacheRunLayers:               ctx.Bool("cache-run-layers"),
		...
		ImageDownloadRetry:           ctx.Int("image-download-retry"),
		...
		PushIgnoreImmutableTagErrors: ctx.Bool("push-ignore-immutable-tag-errors"),
		...
		RegistryMap:                  ctx.StringSlice("registry-map"),
