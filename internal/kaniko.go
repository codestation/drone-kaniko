...
func commandBuild(build Build) *exec.Cmd {
	...
	if build.CacheDir != "" {
		args = append(args, "--cache-dir", build.CacheDir)
	}

	if build.CacheRepo != "" {
		args = append(args, "--cache-repo", build.CacheRepo)
	}

	if build.CacheTTL != nil {
		args = append(args, "--cache-ttl", build.CacheTTL.String())
	}
	...
	if build.Context != "" {
		args = append(args, "--context", build.Context)
	}

	for _, dest := range build.Destinations {
		args = append(args, "--destination", dest)
	}

	if build.DigestFile != "" {
		args = append(args, "--digest-file", build.DigestFile)
	}

	if build.Dockerfile != "" {
		args = append(args, "--dockerfile", build.Dockerfile)
	}

	if build.OutputFile != "" {
		args = append(args, "--image-name-with-digest-file", build.OutputFile)
	}
...
func addProxyValue(build *Build, key string) {
	...
	if len(value) > 0 && !hasProxyBuildArg(build, key) {
		build.BuildArgs = append(build.BuildArgs, fmt.Sprintf("%s=%s", key, value))
		build.BuildArgs = append(build.BuildArgs, fmt.Sprintf("%s=%s", strings.ToUpper(key), value))
	}
}
